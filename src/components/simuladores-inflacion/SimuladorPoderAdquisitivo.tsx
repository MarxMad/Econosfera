...
import { Wallet, FileDown, Save, Info } from "lucide-react";
      ...
      <div className="flex justify-between items-center gap-2 text-[10px] font-black text-slate-500 uppercase">
        ...
        <div className="flex items-center gap-1">
          <input
            ...
            className="w-16 px-2 py-1 text-right font-mono font-bold rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-amber-600 focus:outline-none focus:ring-2 focus:ring-amber-500"
          />
...
export default function SimuladorPoderAdquisitivo() {
  ...
  const handleExport = async () => {
    ...
    setExportando(true);
    try {
      await import("@/lib/actions/exportActions").then((m) => m.registrarExportacion("Inflación Poder Adquisitivo", "PDF"));
      let chartUrl: string | null = null;
      try {
        chartUrl = await import("@/lib/exportarGrafico").then((m) => m.getGraficoAsDataUrl("grafico-poder-adquisitivo"));
      } catch (_) {}
      const { exportarPoderAdquisitivoPdf } = await import("@/lib/exportarPdf");
      await exportarPoderAdquisitivoPdf(principal, nominalRate, inflation, years, stats.finalNominal, stats.finalReal, stats.erosionPercent, chartUrl);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-slate-200 dark:border-slate-700 bg-gradient-to-br from-amber-50 to-orange-50 dark:from-slate-900 dark:to-slate-800/80 p-6 shadow-lg overflow-hidden relative">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Visualiza cómo la inflación erosiona el valor del dinero. Ecuación de Fisher: r ≈ i − π.
              </p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            {session && (
              <button
                ...
                onClick={handleSave}
                disabled={guardando}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold bg-amber-600 text-white hover:bg-amber-500 transition-all shadow-md active:scale-95 disabled:opacity-50"
              >
      ...
      <InstruccionesSimulador>
        <p>El poder adquisitivo mide cuánto puedes comprar con tu dinero. La inflación lo reduce con el tiempo.</p>
        <ul className="list-disc list-inside space-y-1 ml-1">
          <li><strong>Valor nominal:</strong> Tu dinero crece con la tasa nominal (i).</li>
          <li><strong>Valor real:</strong> Cuánto puedes comprar en términos de bienes. Crece con r ≈ i − π.</li>
          <li><strong>Erosión:</strong> Si tu tasa nominal no supera la inflación, pierdes poder adquisitivo cada año.</li>
        </ul>
      </InstruccionesSimulador>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-6 border border-slate-200 dark:border-slate-800 shadow-lg">
            <h3 className="font-black text-slate-400 mb-8 uppercase text-[10px] tracking-widest text-slate-500">Variables</h3>
          ...
          <div className="bg-amber-500 rounded-3xl p-8 text-white shadow-xl shadow-amber-500/20 relative overflow-hidden">
            <h4 className="font-bold text-[10px] uppercase mb-4 tracking-widest opacity-80 flex items-center gap-2">
              <Info className="w-3 h-3" /> Impuesto Inflacionario
            </h4>
            ...
            <div className="mt-8 pt-6 border-t border-amber-400/50 space-y-4">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 border border-slate-200 dark:border-slate-800 shadow-lg h-full min-h-[500px] flex flex-col">
            <h3 className="font-black text-slate-900 dark:text-white text-xl uppercase tracking-tighter mb-10">Crecimiento Nominal vs Real</h3>

            <div id="grafico-poder-adquisitivo" className="flex-1 w-full min-h-[350px]">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  <defs>
                    <linearGradient id="colorNominalPA" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="#3b82f6" stopOpacity={0.1} />
                      <stop offset="95%" stopColor="#3b82f6" stopOpacity={0} />
                    </linearGradient>
                    <linearGradient id="colorRealPA" x1="0" y1="0" x2="0" y2="1">
                  ...
                  <Area name="Crecimiento Nominal" type="monotone" dataKey="nominal" stroke="#3b82f6" strokeWidth={2} fill="url(#colorNominalPA)" />
                  <Area name="Poder de Compra (Real)" type="monotone" dataKey="real" stroke="#10b981" strokeWidth={5} fill="url(#colorRealPA)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>

            <div className="mt-8 flex items-center justify-center gap-8 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-700">
              ...
              <p className="text-xs text-slate-500 max-w-sm leading-relaxed font-medium">
                ...
                Tu dinero solo crece en términos reales si la tasa nominal supera a la inflación.
              </p>
            </div>
          </div>
        </div>
