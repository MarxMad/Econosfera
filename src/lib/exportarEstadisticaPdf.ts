...
export type EstadisticaExportData = {
  tipo: "Regresion" | "TCL";
  titulo: string;
...
export async function exportarEstadisticaAPdf(data: EstadisticaExportData) {
  const doc = new jsPDF();
  ...
  doc.text("ESTADÍSTICA Y ECONOMETRÍA", MARGIN, 27);
  doc.setFontSize(14);
  ...
  if (data.chart && currentY < 200) {
    try {
      ...
      console.warn("Chart image failed for Estadistica", e);
    }
  }
  ...
  doc.save(`Econosfera_Estadistica_${data.tipo}_${new Date().toISOString().split("T")[0]}.pdf`);
}
